...
export function renderEmailShell(input: EmailShellInput): string {
  ...
  const cta = input.cta
    ? `<tr><td style="padding:28px 24px;text-align:center;"><a href="${escapeHtml(input.cta.url)}" style="display:inline-block;background:#e5bd79;color:#18120b;font-size:12px;font-weight:700;letter-spacing:1px;text-transform:uppercase;text-decoration:none;padding:14px 30px;border-radius:4px;">${escapeHtml(input.cta.label)}</a></td></tr>`
    : '';
  const fallbackUrl = input.cta
    ? `<p style="margin:16px 0 0;font-size:11px;line-height:1.6;word-break:break-all;color:#8b867d;">If the button does not work, copy this link:<br/><a href="${escapeHtml(input.cta.url)}" style="color:#e3bb78;">${escapeHtml(input.cta.url)}</a></p>`
    : '';
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0b;padding:28px 12px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#111110;border:1px solid #2d2a27;border-radius:6px;">
        <tr><td style="padding:26px 24px 0;text-align:center;"><p style="margin:0;font-size:11px;font-weight:700;letter-spacing:3px;color:#e0bd7d;">ELEVATE APPAREL</p></td></tr>
