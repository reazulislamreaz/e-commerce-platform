...
| `POST /api/v1/auth/register`                    | Implemented | Creates an ACTIVE account (fullName, email, BD phone, password), sends welcome email; duplicate email/phone → 409                                               |
| `GET /api/v1/auth/verify-email`                 | Implemented | Legacy: consumes an old emailed verification token for PENDING accounts; sets `emailVerifiedAt` and activates                                                   |
...
| `GET /api/v1/users`                             | Implemented | Offset-paginated directory (search/status/role/date/verified/sort/deleted); admins see customers only; enriched with verification, last login, orders, spending |
| `GET /api/v1/users/:id`                         | Implemented | Admins may read customer accounts only                                                                                                                          |
| `GET /api/v1/users/:id/detail`                  | Implemented | Full admin profile: addresses, orders, wishlist, reviews, activity, login history, notes, audit trail                                                           |
| `PATCH /api/v1/users/:id`                       | Implemented | Edit name/phone                                                                                                                                                 |
| `PATCH /api/v1/users/:id/notes`                 | Implemented | Internal admin notes                                                                                                                                            |
...
| `POST /api/v1/users/:id/verify`                 | Implemented | Mark email verified; activates pending accounts                                                                                                                 |
| `POST /api/v1/users/:id/reset-password`         | Implemented | Queue password-reset email for active accounts                                                                                                                  |
| `POST /api/v1/users/:id/restore`                | Implemented | Restore soft-deleted account from preserved contact fields                                                                                                      |
| `POST /api/v1/users/bulk`                       | Implemented | Bulk activate/suspend/verify/soft-delete/restore                                                                                                                |
| `DELETE /api/v1/users/:id`                      | Implemented | Soft delete + contact preservation for restore + session revocation                                                                                             |
...
- Registration requires full name, email, password (min 6 characters, any characters), and a unique Bangladeshi mobile number (accepted as `01XXXXXXXXX` or `+8801XXXXXXXXX`, stored in E.164; see `common/utils/bd-phone.ts`). Accounts are created `ACTIVE` immediately and receive a welcome email. Signup no longer requires email verification; `GET /auth/verify-email` and `POST /auth/resend-verification` remain for legacy PENDING accounts. There is no SMS/phone verification.
- Password recovery uses the `verification_token` table with type `PASSWORD_RESET`: single-use emailed link, 30-minute expiry, newest link supersedes older ones, and a successful reset revokes every session. Password change (authenticated) requires the current password and revokes all other sessions. The password policy (6–128 chars, any characters) is shared via `common/decorators/is-account-password.decorator.ts`.
